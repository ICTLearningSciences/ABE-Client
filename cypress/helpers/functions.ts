...
//   }
  ...
  export function cyMockDefault(
    ...
    cyInterceptGraphQL(cy, [
      ...gqlQueries,
      //Defaults
      mockGQL("RefreshAccessToken", refreshAccessTokenResponse(args.userRole || UserRole.USER)),
  ...
  export function toStrongerHookActivity(cy: CypressGlobal, step?: StepNames){
    ...
    if(step === StepNames.Improve_Narrativity || step === StepNames.Narrativity_Brainstorm || step === StepNames.Narrativity_Story_In_Mind || step === StepNames.Narrativity_Outro){
      ...
      if(step === StepNames.Narrativity_Outro){
        cy.get("[data-cy=mcq-choice-Brainstorm-stories]").click()
        cyMockOpenAiCall(cy, {response: entityFoundResponse([{experience: "my brother", interest: 4, justification:"", question: ""}])})
    ...
    if(step === StepNames.Improve_Emotions || step === StepNames.Complete_Emotions){
      cyMockOpenAiCall(cy, {response: analyzeHookResponse(2,2)})
      cy.get("[data-cy=mcq-choice-Ready]").click();
      cy.get("[data-cy=mcq-choice-Improve-emotion]").click()

      if(step === StepNames.Complete_Emotions){
        cyMockOpenAiCall(cy, {response: audienceEmotionsResponse([{name: "students", emotions: ["anger", "sadness"]}, {name: "president of the united states", emotions: ["hopefulness"]}])})
        cy.get("[data-cy=chat-input]").type("I want to speak to students and convey anger and sadness.")
        cy.get("[data-cy=send-input-button]").click();

        // what revision stage
        cy.get("[data-cy=messages-container]").should("contain.text", "What kind of revision are you thinking of doing now?")
        cy.get("[data-cy=chat-input]").type("I want to talk about the president.")
        cy.get("[data-cy=send-input-button]").click();

        // re-analyze after revision stage
        cy.get("[data-cy=messages-container]").should("contain.text", "Let me know when you're done revising")
        cyMockOpenAiCall(cy, {response: openAiTextResponse("You revised your paper well.")})
        cy.get("[data-cy=mcq-choice-Ready]").click();
      }
    }
  ...
  export function toPromptActivity(cy: CypressGlobal){
    ...
    const cookie = cy.getCookie("refreshTokenDev")
    cy.visit("/docs/1W8nTQk1bmzs88L-nxjIhCIqqAVVdzJRvzqRVV4v4lqg");
    cy.get("[data-cy=goal-display-6580e5640ac7bcb42fc8d27f]").click();
