...
import {
  ...
  eightHoursBetweenSessionsDocVersions,
} from "../fixtures/document-timeline/eight-hours-difference";
import { fetchActivitiesResponse } from "../fixtures/fetch-activities";
import {
  createActivityBuilder,
  fetchBuiltActivitiesResponse,
} from "../fixtures/fetch-built-activities";
...
import { MockDefaultType, testGoogleDocId } from "./types";
...
export function cyMockDefault(
  ...
  cyInterceptGraphQL(cy, [
    ...
    mockGQL("StoreBuiltActivityVersion", {
      storeBuiltActivityVersion: { activity: {} },
    }),
    mockGQL("SubmitDocVersion", {}),
    mockGQL(
      "CopyBuiltActivity",
      {
        copyBuiltActivity: createActivityBuilder(
          testUser._id,
          "Copied Activity",
          "copied-activity",
          "editable",
...
export function cyMockGoogleDoc(cy: CypressGlobal) {
...
export function toMyEditableActivity(cy: CypressGlobal) {
  cy.getLocalStorage(ACCESS_TOKEN_KEY)
    .should("exist")
    .should("equal", "fake-access-token");
  cy.getCookie("refreshTokenDev")
    .should("exist")
...
export function roleSwitch(cy: CypressGlobal, targetNewRole: UserRole) {
  ...
  cy.get("[data-cy=role-switch]").should(
    "contain.text",
