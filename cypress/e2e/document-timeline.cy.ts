...
} from "../fixtures/document-timeline/generations-in-progress";
...
describe("document timeline", () => {
  ...
  describe("summary and outline generation in progress", () => {
    it("shows loading status if generation not complete", () => {
      ...
      cy.get("[data-cy=timeline-footer-item-card-0]").should(
        "have.css",
        "opacity",
        "0.5",
      );
      cy.get("[data-cy=timeline-footer-item-card-1]").should(
        "have.css",
        "opacity",
        "0.5",
      );
    ...
    it("can view other parts of the document while generation is in progress", () => {
      ...
      cy.get("[data-cy=ai-summary-in-progress]").should("exist");
      cy.get("[data-cy=ai-outline-in-progress]").should("exist");
      cy.get("[data-cy=intention-container]").should(
        "contain.text",
        "This activity is to work on the hook of the essay.",
      );
      cy.get("[data-cy=assignment-container]").should(
        "contain.text",
        "Aliens assignment description",
      );
    });
    ...
    it("properly displays summary and outline when generation is complete", () => {
      ...
      cy.wait("@FetchDocumentTimelineStatus", { timeout: 3000 });
      cy.get("[data-cy=summary-container]").should(
        "contain.text",
        "Complete Summary",
      );
    ...
    it("stops polling when generation is complete", () => {
      ...
      cy.wait(2000);
      cy.get("@FetchDocumentTimelineStatus.all")
        .its("length")
        .then((len) => {
          cy.wait(4000);
