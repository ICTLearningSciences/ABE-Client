...
} from "../fixtures/educational-management/assignment-operations";
import {
  createNewStudentWithIncompleteActivityResponse,
  fetchCoursesResponseEmpty,
  fetchCoursesResponseStudent,
  removeFromSectionResponse,
  updateTestSectionWithAssignmentsResponse,
} from "../fixtures/educational-management";
import { fetchDocVersionsBuilder } from "../fixtures/fetch-doc-versions-builder";
import {
  realExampleDocumentTimeline2,
  realExampleDocVersions,
  realExampleDocVersions2,
} from "../fixtures/document-timeline/real-example";
import { openAiTextResponse } from "../fixtures/stronger-hook-activity/basic-text-response";
import { studentWithUpdatedActivityDefaultLLM } from "../fixtures/educational-management/assignment-progress-operations";
...
describe("Course Management", () => {
  describe("Initial View", () => {
    it("Instructors see course management page on login", () => {
      ...
      cy.get("[data-cy=course-management-title]").should(
        "contain.text",
        "Course Management",
      );
      cy.get("[data-cy=course-management-description]").should(
        "contain.text",
  ...
  describe("Course CRUD Operations", () => {
    it("Instructors can CRUD a new course", () => {
      ...
      cy.get("[data-cy=course-management-main-title]").should(
        "contain.text",
        "Course Management",
      );
      ...
      // Modify the course
      cy.get("[data-cy=course-title-input]")
        .clear()
        .type("Updated Test Course");
      cy.get("[data-cy=course-modal-submit-button]").click();
  ...
  describe("Section CRUD Operations", () => {
    it("Instructors can CRUD a new section", () => {
      ...
      cy.get("[data-cy=section-description-input]").type(
      ...
      cy.get("[data-cy=section-card-new-section-123]")
        .scrollIntoView()
        .should("be.visible")
        .click();
      ...
      // Modify the section
      cy.get("[data-cy=section-title-input]")
        .clear()
        .type("Updated Test Section");
      cy.get("[data-cy=section-description-input]")
        .clear()
        .type("This is an updated test section description");
  ...
  describe("Assignment CRUD Operations", () => {
    it("Instructors can CRUD a new assignment", () => {
      ...
      cy.get("[data-cy=assignment-modal-description-input]").type(
        "This is a test assignment description",
      );
      ...
      cy.get("[data-cy=assignment-card-new-assignment-123]")
        .should("be.visible")
        .click();
      ...
      cy.get("[data-cy=edit-assignment-button]")
        .scrollIntoView()
        .should("be.visible")
        .click();
      cy.get("[data-cy=assignment-modal]").should("be.visible");
      cy.get("[data-cy=assignment-modal-advanced-toggle]").click();
      cy.get("[data-cy=assignment-modal-title-input]")
        .clear()
        .type("Updated Test Assignment");
      cy.get("[data-cy=assignment-modal-description-input]")
        .clear()
        .type("This is an updated test assignment description");
      ...
      cy.wait("@AddOrUpdateAssignment").then((xhr) => {
        ...
        expect(data.assignmentData.defaultLLM.model).to.equal(
          "gpt-3.5-turbo-16k",
        );
      });
      ...
      cy.get("[data-cy=assignment-card-new-assignment-123]").should(
        "not.exist",
      );
    });
  });
    ...
    it("Students can remove themselves from a section", () => {
      cyMockEducationalManagement(cy, {
        ...
        gqlQueries: [
          ...
          mockGQL("ModifySectionEnrollment", [removeFromSectionResponse]),
        ],
      });
      ...
      cy.get("[data-cy=remove-from-section-button]")
        .should("be.visible")
        .click();
  ...
  describe("Activity Progress", () => {
    ...
    it("student can set default LLM for an activity, overrides course default LLM", () => {
      ...
      cy.wait("@openAiStartCall").then((xhr) => {
        ...
        expect(aiStep.targetAiServiceModel).to.exist;
        expect(aiStep.targetAiServiceModel.serviceName).to.equal("OPEN_AI");
        expect(aiStep.targetAiServiceModel.model).to.equal(
          "gpt-4-turbo-preview",
        );
      });
    });
  ...
  describe("Doc CRUD Operations", () => {
    ...
    it("can set primary doc", () => {
      ...
      cy.get("[data-cy=activity-item-my-editable-activity]").click();
      cy.get("[data-cy=primary-doc-checkbox-Aliens]").should(
        "not.have.class",
        "Mui-checked",
      );
  ...
  describe("Student Timeline", () => {
    it("Can view students timeline", () => {
      ...
      cy.get("[data-cy=assignment-header-assignment-title]").should(
        "contain.text",
        "Programming Fundamentals Quiz",
      );

      // Timeline Notches Visible
      cy.get("[data-cy=activity-document-timeline-notches]").should(
        "be.visible",
      );
      cy.get("[data-cy=activity-document-timeline-notch-0]").should(
        "be.visible",
      );
      cy.get("[data-cy=activity-document-timeline-notch-0]").should(
        "contain.text",
        "My Editable Activity",
      );
      cy.get("[data-cy=activity-document-timeline-notch-1]").should(
        "contain.text",
        "+407",
      );
      ...
      cy.get("[data-cy=text-view-content]").should(
        "contain.text",
      ...
      cy.get("[data-cy=text-view-content]").should(
        "not.contain.text",
        "These culturally significant practices",
      );
      cy.get("[data-cy=ai-outline-tab-selector]").click();
      cy.get("[data-cy=ai-outline-container]").should(
        "not.contain.text",
        "Federal response to wildfires is insufficient",
      );
    ...
    it("can switch between docs", () => {
      ...
      cy.get(
        "[data-value=1LqProM_kIFbMbMfZKzvlgaFNl5ii6z5xwyAsQZ0U87Y]",
      ).click();
    ...
    it("Can grade student assignment", () => {
      ...
      cy.get("[data-cy=not-graded-assignment]")
        .should("exist")
        .should("be.visible");
