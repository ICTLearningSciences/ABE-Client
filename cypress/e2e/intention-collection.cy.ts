...
*/
import { gDocWithAllIntentions, gDocWithExpiredDayIntention, gDocWithoutAssignmentDescription, gDocWithoutCurrentDayIntentionAndExpiredDocumentIntention, gDocWithoutDocumentIntention, storeGoogleDocResponse } from "../fixtures/intentions/google-docs-intentions";
import { CypressGlobal, StepNames, cyMockDefault, mockGQL, sendChatMessage, testGoogleDocId, toStrongerHookActivity } from "../helpers/functions"
...
function writeDocumentIntention(cy: CypressGlobal ,input: string){
    cy.get("[data-cy=input-document-intention]").clear().type(input, {delay: 0});
}
...
function isOnSelectGoalScreen(cy: CypressGlobal){
    cy.get("[data-cy=doc-goal-modal]").should("contain.text", "What is your current goal?")
}
...
function didDocDataGetSaved(cy: CypressGlobal, docIntention?: string, docAssignment?: string, dayIntention?: string){
    cy.wait("@StoreGoogleDoc").then((xhr)=>{
        const variablesSent = xhr.request.body.variables;
        if(docIntention){
            expect(variablesSent.googleDoc.documentIntention.description).to.eql(docIntention);
        }else{
            expect(variablesSent.googleDoc.documentIntention).to.eql(undefined)
        }

        if(docAssignment){
            expect(variablesSent.googleDoc.assignmentDescription).to.eql(docAssignment);
        }else{
            expect(variablesSent.googleDoc.assignmentDescription).to.eql(undefined)
        }

        if(dayIntention){
            expect(variablesSent.googleDoc.currentDayIntention.description).to.eql(dayIntention);
        }else{
...
function didDocVersionDataGetSaved(cy: CypressGlobal, sessionIntention?: string, docIntention?: string, dayIntention?: string){
    ...
    cy.get("@SubmitGoogleDocVersion.all").then((xhr)=>{
        ...
        const variablesSent: Partial<DocVersion> = lastRequest.request.body.variables.googleDocData;
        if(docIntention){
        ...
        if(sessionIntention){
            expect(variablesSent.sessionIntention?.description).to.eql(sessionIntention);
        }else{
...
function checkNumberSessionIds(cy: CypressGlobal, num: number){
    cy.wait(6000); //to allow next version to be submitted
    cy.get("@SubmitGoogleDocVersion.all").then((xhr)=>{
        const sessionIds = xhr.map((x: any)=>x.request.body.variables.googleDocData.sessionId);
    ...
    describe("document intention", ()=>{
        it("collects and saves document intention on first visit for document", ()=>{
            cyMockDefault(cy,
                {
                    gqlQueries: [
                        mockGQL("FetchGoogleDocs", {fetchGoogleDocs: [gDocWithoutDocumentIntention]}),
    ...
    describe("day intention", ()=>{
        it("does not collect day intention for new document within 8 hours of creation", ()=>{
            cyMockDefault(cy,
                {
                    gqlQueries: [
                        mockGQL("FetchGoogleDocs", {fetchGoogleDocs: [gDocWithAllIntentions]}),
                        mockGQL("StoreGoogleDoc", storeGoogleDocResponse(gDocWithAllIntentions)),
                    ]
                });
                cy.visit(`/docs/${testGoogleDocId}`);
        ...
        it("collects and saves day intention for new document after 8 hours of creation", ()=>{
            cyMockDefault(cy,
                {
                    gqlQueries: [
                        mockGQL("FetchGoogleDocs", {fetchGoogleDocs: [gDocWithoutCurrentDayIntentionAndExpiredDocumentIntention]}),
                        mockGQL("StoreGoogleDoc", storeGoogleDocResponse(gDocWithAllIntentions)),
                    ]
                }
            );
        ...
        it("collects and saves day intention if 8 hours after last collection", ()=>{
            cyMockDefault(cy,
                {
                    gqlQueries: [
                        mockGQL("FetchGoogleDocs", {fetchGoogleDocs: [gDocWithExpiredDayIntention]}),
                        mockGQL("StoreGoogleDoc", storeGoogleDocResponse(gDocWithAllIntentions)),
                    ]
                }
            );
    ...
    describe("document assignment", ()=>{
        it("collects and saves document assignment on first visit for document", ()=>{
            cyMockDefault(cy,
                {
                    gqlQueries: [
                        mockGQL("FetchGoogleDocs", {fetchGoogleDocs: [gDocWithoutAssignmentDescription]}),
                        mockGQL("StoreGoogleDoc", storeGoogleDocResponse(gDocWithAllIntentions)),
                    ]
                });
            cy.visit(`/docs/${testGoogleDocId}`);
    ...
    describe("session intention (in memory only)", ()=>{
        describe("stronger hook activity", ()=>{
            it("collects and saves session intention when asked what they'd like to revise", ()=>{
                ...
                didDocVersionDataGetSaved(cy, "This is my session intention.", "Aliens document intention", "Aliens day intention");
            })
        })
