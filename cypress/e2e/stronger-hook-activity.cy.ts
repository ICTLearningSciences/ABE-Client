...
describe('Stronger Hook Activity', () => {
    it('Should show introduction text and ask if ready', () => {
      ...
      cy.get("[data-cy=messages-container]").should("contain.text", "Feel free to edit the intro to your paper");
    ...
    it("activity should reset when going to another activity", ()=>{
      ...
      cy.get("[data-cy=messages-container]").should("contain.text", "Your hook lacks any narrativity.")
    ...
    it("openai request fails retries 3 times before giving up", ()=>{
      ...
      cy.get("[data-cy=mcq-choice-Ready]").click();
      cy.wait(3000)
      cy.get("[data-cy=messages-container]").should("contain.text", "Request failed, please try again later.")
    })
    ...
    it("openai request retries and safely continues activity", ()=>{
      ...
      cy.get("[data-cy=messages-container]").should("contain.text", "Your hook lacks any narrativity.")
      cy.get("[data-cy=messages-container]").should("contain.text", "What would you like to work on?")
      ...
      cy.get("[data-cy=messages-container]").should("contain.text", "Would you like to brainstorm some stories or do you already have a story in mind?")
    ...
    describe("low narrativity and low emotion", ()=>{
      it("walk through narrative brainstorm", () =>{
        ...
        cyMockOpenAiCall(cy, {response: entityFoundResponse([{experience: "my brother", interest: 4, justification: "justification", question:"question"} ])})
        cy.get("[data-cy=chat-input]").type("My boss, Hector, and my brother. Also my dog.")
        cy.get("[data-cy=send-input-button]").click();
        
        cy.get("[data-cy=messages-container]").should("contain.text", "This is the entity found response.")
        cy.get("[data-cy=messages-container]").should("contain.text", "Would you like to brainstorm more examples or work with what you've got?")
        cy.get("[data-cy=mcq-choice-Brainstorm-More]").should("exist")
        cy.get("[data-cy=mcq-choice-Work-with-what-I-have]").click()

        // analyze story stage
        cyMockOpenAiCall(cy, {response: openAiTextResponse("Here are my thoughts on your story:\n1.adaptation\n2.foreshadowing\n3.character development")})

        cy.get("[data-cy=chat-input]").type("I think my dog Clifford is an Alien.")
        cy.get("[data-cy=send-input-button]").click();
        cy.get("[data-cy=messages-container]").should("contain.text", "Here are my thoughts on your story")

        
        // revision stage
        cyMockOpenAiCall(cy, {response: openAiTextResponse("You revised your paper well.")})
        cy.get("[data-cy=messages-container]").should("contain.text", "Here are my thoughts on your story")
        ...
        cy.get("[data-cy=send-input-button]").click();
        cy.get("[data-cy=messages-container]").should("contain.text", "Please revise your paper")
        cy.get("[data-cy=mcq-choice-Ready]").click()

        cy.get("[data-cy=messages-container]").should("contain.text", "You revised your paper well.")
        cy.get("[data-cy=messages-container]").should("contain.text", "What would you like to do next?")
        cy.get("[data-cy=mcq-choice-Improve-emotion]").should("exist")
        cy.get("[data-cy=mcq-choice-Improve-narrativity]").should("exist")
      ...
      it("outro analyze again restarts activity", ()=>{
        ...
        // outro
        cy.get("[data-cy=messages-container]").should("contain.text", "Okay, would you like to revise and then have me analyze it again?")
      ...
      it("outro continue as open dialogue goes to free input", ()=>{
        ...
        cy.get("[data-cy=messages-container]").should("contain.text", "Okay, feel free to ask me anything you'd like")
      ...
      it("if no entities found, then ask to try again", ()=>{
        ...
        cy.get("[data-cy=send-input-button]").click();
        cy.get("[data-cy=messages-container]").should("contain.text", "I didn't find any people or places in your responses. Please try again.")
        cy.get("[data-cy=chat-input]").type("i would")
      })
      ...
      it("Help me brainstorm button", ()=>{
        ...
        cy.get("[data-cy=messages-container]").should("contain.text", "Example help brainstorm response.")
        cy.get("[data-cy=messages-container]").should("contain.text", "If these helped at all, please brainstorm some people ")

        cyMockOpenAiCall(cy, {response: entityFoundResponse([{experience:"Barack Obama", interest: 4, justification:"", question:""}])})
        cy.get("[data-cy=chat-input]").type("My brother, my mom. Barack Obama and Trump.")
        cy.get("[data-cy=send-input-button]").click();
      ...
      it("emotion pumps for at least 2 audience members", ()=>{
        ...
        cy.get("[data-cy=messages-container]").should("contain.text", "Great. Consider who this piece is speaking to.")
        cyMockOpenAiCall(cy, {response: audienceEmotionsResponse([{name: "students", emotions: ["anger", "sadness"]}])})
        cy.get("[data-cy=chat-input]").type("I want to speak to students and convey anger and sadness.")
        cy.get("[data-cy=send-input-button]").click();


        // asking for more audience members
        cy.get("[data-cy=messages-container]").should("contain.text", "Okay. Are there any other audiences or stakeholders who are important to this? How should they feel?")
        cyMockOpenAiCall(cy, {response: audienceEmotionsResponse([{name: "president of the united states", emotions: ["hopefulness"]}])})
        cy.get("[data-cy=chat-input]").type("I would also like to speak to the president of the united states and convey hopefulness.")
        cy.get("[data-cy=send-input-button]").click();
      ...
      it("pending openai calls does not botch swapping activities", ()=>{
        cyMockDefault(cy);
        cyMockOpenAiCall(cy, {response: analyzeHookResponse(2,5), delay: 3000})
        toStrongerHookActivity(cy);
        ...
        cy.wait(4000)
        cy.get("[data-cy=messages-container]").should("not.contain.text", "Would you like to make this a more narrative hook?")
      ...
      it("can reset activity", ()=>{
        cyMockDefault(cy);
        toStrongerHookActivity(cy, StepNames.Improve_Emotions);
        cy.get("[data-cy=chat-box]").should("contain.text", "Great. Consider who this piece is speaking to.")
        cy.get("[data-cy=reset-activity-button]").click()
        cy.get("[data-cy=chat-box]").should("not.contain.text", "Great. Consider who this piece is speaking to.")
        cy.get("[data-cy=chat-box]").should("contain.text", "Feel free to edit the intro to your paper, and tell me when it's ready for me to review.")
        cy.get("[data-cy=mcq-choice-Ready]").click();
        cy.get("[data-cy=messages-container]").should("contain.text", "Your hook lacks any narrativity.")
      })
