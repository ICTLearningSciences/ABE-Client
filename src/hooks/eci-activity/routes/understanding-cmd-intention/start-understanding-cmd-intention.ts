  ...
  return {
    ...
    handleResponse: async (userInterpretationOfCmdIntent) => {
      ...
      await executePrompt(
        () => updatedPrompt,
        (res) => {
          const compareInterpretationRes =
            validateJsonResponse<CompareAnalysisRes>(
              res.answer,
...
export function pickQuestionAboutInterpretation(
  ...
  return {
    text:
      pickQuestionMessage || 'Pick one of these questions to explore in depth',
...
/** free discussion */
const REVISE_INTERPRETATION =
  'Revise my Interpretation of Commanders Intention';
